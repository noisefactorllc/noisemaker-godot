// check_validate.mjs — VALIDATOR parity gate. For every corpus + program DSL, compare the GDScript
// validator's output (candidate, via _validate_dump.gd) against the REFERENCE validator (oracle, via
// dump-validate.mjs), both fed effects/ops/enums/aliases/starters from the SAME effect JSONs the way
// tools/export-graph.mjs does. Key-order-insensitive deep value-compare per file; numbers compared
// under a tight relative epsilon (Godot's JSON float serialization loses ~1 ULP — see check_parse).
//
//   NM_REFERENCE_ROOT=/path/to/noisemaker GODOT=/path/to/Godot node parity/check_validate.mjs [file...]
...
const candRaw = execFileSync(GODOT, ['--headless', '--path', join(REPO, 'godot'),
    '--script', 'res://addons/noisemaker/compiler/_validate_dump.gd', '--', ...files],
    { encoding: 'utf8', maxBuffer: 1 << 28 })
const markerLine = candRaw.split('\n').find(l => l.startsWith('VALIDATEDUMP:'))
if (!markerLine) { console.error('no VALIDATEDUMP output. tail:\n' + candRaw.slice(-2000)); process.exit(2) }
const cand = JSON.parse(markerLine.slice('VALIDATEDUMP:'.length))
...
console.log(`VALIDATE PARITY: ${pass}/${files.length} pass`)
