// check_graph.mjs — GRAPH parity gate. The real target: the in-engine GDScript compiler builds the
// SAME normalized render graph the reference produces (the shape the backend consumes). For every
// corpus + program DSL, compare the GDScript orchestrator's graph (candidate, via _graph_dump.gd)
// against reference normalizeGraph(compileGraph(dsl)) (oracle, via dump-graph.mjs), both over the
// same effect JSONs. Key-order-insensitive deep compare; numbers under a tight relative epsilon.
//
//   NM_REFERENCE_ROOT=/path/to/noisemaker GODOT=/path/to/Godot node parity/check_graph.mjs [file...]
...
const candRaw = execFileSync(GODOT, ['--headless', '--path', join(REPO, 'godot'),
    '--script', 'res://addons/noisemaker/compiler/_graph_dump.gd', '--', ...files],
    { encoding: 'utf8', maxBuffer: 1 << 28 })
const markerLine = candRaw.split('\n').find(l => l.startsWith('GRAPHDUMP:'))
if (!markerLine) { console.error('no GRAPHDUMP output. tail:\n' + candRaw.slice(-2000)); process.exit(2) }
const cand = JSON.parse(markerLine.slice('GRAPHDUMP:'.length))
...
console.log(`GRAPH PARITY: ${pass}/${files.length} pass`)
