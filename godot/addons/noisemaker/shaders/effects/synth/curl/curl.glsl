#version 450
// synth/curl — ported PIXEL-IDENTICALLY from wgsl/curl.wgsl (top-left origin =
// Godot/Vulkan, no Y-flip). 3D curl noise built on Stefan Gustavson's 3D simplex
// noise. OCTAVES, RIDGES, OUTPUT_MODE are compile-time #define ints injected by
// the backend (definition globals.{octaves,ridges,outputMode}.define) — kept as
// bare identifiers. Packed uniformLayout: vec4 data[4] (effects/synth/curl.json).
//
// Helpers (permute3/4, taylorInvSqrt, simplex3D, fbmSimplex3D, curlNoise3D) are
// VERBATIM INLINE from the WGSL — no shared-lib substitution. nm_core is included
// only to honor the porting convention; curl uses none of its primitives.
...
// WGSL Uniforms struct fields referenced by helpers (set from data[] in main).
float u_time;
float u_scale;
int u_seed;
float u_speed;
...
float simplex3D(vec3 v) {
	...
	// Permutations
	vec3 iMod = mod(i, 289.0);
	vec4 p = permute4(permute4(permute4(
		iMod.z + vec4(0.0, i1.z, i2.z, 1.0))
		+ iMod.y + vec4(0.0, i1.y, i2.y, 1.0))
		+ iMod.x + vec4(0.0, i1.x, i2.x, 1.0));
	...
	m = m * m;
	return 42.0 * dot(m * m, vec4(dot(p0n, x0), dot(p1n, x1), dot(p2n, x2), dot(p3n, x3)));
}
...
vec3 curlNoise3D(vec3 p) {
	...
	float a = (sin(u_time * 6.28318) * (u_speed) + 1.0) / float(OCTAVES) * 0.2;
	float b = (cos(u_time * 6.28318) * (u_speed) + 1.0) / float(OCTAVES) * 0.2;
...
void main() {
	...
	u_seed = int(data[1].y);
	u_speed = data[1].z;
	u_intensity = data[2].z;
	u_tileOffset = data[3].xy;
	u_fullResolution = data[3].zw;

	vec2 uv = (gl_FragCoord.xy + u_tileOffset) / u_fullResolution;
	float aspect = u_fullResolution.x / u_fullResolution.y;

	// Center and scale coordinates
	vec2 centered = (uv - 0.5) * vec2(aspect, 1.0);
	vec3 p = vec3(centered * (21.0 - u_scale), 0.5);

	// Compute 3D curl noise
	vec3 curl = curlNoise3D(p);

	// Smooth compression to [0, 1] — tanh saturates gracefully, intensity controls curve
	vec3 curlNorm = tanh(curl * u_intensity) * 0.5 + 0.5;

	vec3 color;

	if (OUTPUT_MODE == 0) {
		// flowX: curl.x component
		color = vec3(curlNorm.x);
	} else if (OUTPUT_MODE == 1) {
		// flowY: curl.y component
		color = vec3(curlNorm.y);
	} else if (OUTPUT_MODE == 2) {
		// flowZ: curl.z component
		color = vec3(curlNorm.z);
	} else if (OUTPUT_MODE == 3) {
		// full: all three components as RGB
		color = curlNorm;
	} else {
		// magnitude: length of curl vector
		vec3 curlCentered = curlNorm * 2.0 - 1.0; // Back to [-1, 1]
