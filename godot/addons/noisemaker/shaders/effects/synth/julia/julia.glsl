#version 450
// synth/julia — ported PIXEL-IDENTICALLY from wgsl/julia.wgsl (top-left origin =
// Godot/Vulkan, no Y-flip). Julia set explorer with deep zoom using df64
// double-float emulation. Grayscale value output [0,1], alpha = 1.0.
// Packed uniformLayout: vec4 data[7] (effects/synth/julia.json). No shared
// primitives used; PI/TAU/BAILOUT/LOG2 are inlined per-effect.
...
vec2 df64_mul(vec2 a, vec2 b) {
	float p = a.x * b.x;
	vec2 as_ = df64_split(a.x);
	vec2 bs = df64_split(b.x);
...
vec2 resolveC(int poi, int cPath, float time, float cSpeed, float cRadius, float cReal, float cImag) {
...
void transformCoords(vec2 fragCoord, vec2 resolution,
	...
	imDF = df64_add(df64_mul_f(df64_from(uv.y), scale), df64_from(cy));
}
...
JuliaResult juliaIterate(vec2 z0Re, vec2 z0Im, vec2 c,
...
float iterateSmooth(vec2 fragCoord, vec2 c, int maxIter,
					vec2 resolution, float cx, float cy, float zm, float rot) {
	vec2 zRe, zIm;
	transformCoords(fragCoord, resolution, cx, cy, zm, rot, zRe, zIm);
	float i = 0.0;
	float bail2 = BAILOUT * BAILOUT;
...
float outputNormalMap(vec2 fragCoord, vec2 c, int maxIter, float angle,
					  vec2 resolution, float cx, float cy, float zm, float rot) {
	float d0 = iterateSmooth(fragCoord, c, maxIter, resolution, cx, cy, zm, rot);
	float d1 = iterateSmooth(fragCoord + vec2(1.0, 0.0), c, maxIter, resolution, cx, cy, zm, rot);
	float d2 = iterateSmooth(fragCoord + vec2(0.0, 1.0), c, maxIter, resolution, cx, cy, zm, rot);
...
void main() {
	...
	float cReal = data[1].x;
	float cImag = data[1].y;
	int poi = int(data[1].z);
	int outputMode = int(data[1].w);

	float centerX = data[2].x;
	float centerY = data[2].y;
	float rotation = data[2].z;

	int iterations = int(data[3].x);
	float stripeFreq = data[3].y;
	int trapShape = int(data[3].z);
	float lightAngle = data[3].w;

	int cPath = int(data[4].x);
	float cSpeed = data[4].y;
	float cRadius = data[4].z;
	float invertFlag = data[4].w;

	float zoomSpeed = data[5].x;
	float zoomDepth = data[5].y;

	vec2 tileOffset = data[6].xy;
	vec2 fullResolution = data[6].zw;

	// Resolve c-value
	vec2 c = resolveC(poi, cPath, time, cSpeed, cRadius, cReal, cImag);
	...
	if (outputMode == 4) {
		value = outputNormalMap(gl_FragCoord.xy + tileOffset, c, iterations, lightAngle, fullResolution, centerX, centerY, effectiveZoom, rotation);
	} else {
		vec2 reDF, imDF;
		transformCoords(gl_FragCoord.xy + tileOffset, fullResolution, centerX, centerY, effectiveZoom, rotation, reDF, imDF);
