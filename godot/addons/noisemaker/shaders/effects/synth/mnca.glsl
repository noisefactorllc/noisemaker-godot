#version 450
// synth/mnca — program "mnca" (the display / render pass). Ported from wgsl/mnca.wgsl
// (top-left origin = Godot/Vulkan, no Y-flip). Reads the MNCA state buffer and upscales
// it to output resolution with selectable smoothing (0 nearest, 1 bilinear, 2 cosine,
// 3 catmull-rom 3x3, 4 catmull-rom 4x4, 5 b-spline 3x3, 6 b-spline 4x4). Mono output
// (reads the .g channel). This render pass is byte-identical to synth/ca's render pass;
// only the shared-layout array size differs.
//
// SINGLE shared layout: vec4 data[6] (effects/synth/mnca.json uniformLayout, max slot 5).
// This pass reads only resolution = data[0].xy and smoothing = int(data[1].y).
// gl_FragCoord top-left, +0.5 pixel-centered, no Y-flip.
// The render graph lists four inputs (fbTex/prevFrameTex/bufTex/seedTex) but the shader
// samples ONLY fbTex; the backend binds samplers by name and the SPIR-V compiler strips
// unused ones, so declaring any other sampler would break uniform-set creation. The WGSL
// `mySampler` folds into the combined sampler2D fbTex (NEAREST, no mipmaps — LOD always 0,
// so textureSampleLevel(...,0.0) -> plain texture()).
