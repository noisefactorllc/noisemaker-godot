#version 450
// filter/degauss — ported PIXEL-IDENTICALLY from wgsl/degauss.wgsl. Simulates a CRT
// degaussing wobble by lens-warping each color channel independently along a simplex-
// noise-derived displacement, masked by a center-weighted singularity. Single render pass
// (progName "degauss").
//
...
const float DG_TAU = 6.28318530717958647692;
...
float periodic_value(float timeArg, float value) {
	return normalized_sine((timeArg - value) * DG_TAU);
}
...
float warped_channel_value(
	...
	float noise_value = compute_noise_value(coord, width, height, freq, timeArg, speedArg, channel);
	float centered = (noise_value * 2.0 - 1.0) * mask;
	float angle = centered * DG_TAU;
	vec2 offset = vec2(cos(angle), sin(angle)) * dispArg * vec2(width, height);

	// Rotate offset by direction
	float dirRad = direction * DG_TAU / 360.0;
	float dc = cos(dirRad);
	float ds = sin(dirRad);
	...
	if (channel == 1u) {
		return clamp01(sampled.y);
	}
	return clamp01(sampled.z);
}

void main() {
	vec2 fragId = floor(gl_FragCoord.xy);
	uvec2 gid = uvec2(fragId);

	ivec2 coords = ivec2(gid);
	vec4 original = texelFetch(inputTex, coords, 0);
