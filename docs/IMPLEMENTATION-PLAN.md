...
- [ ] Copy `../noisemaker-hlsl/reference/` → `reference/` (verbatim; engine-agnostic brain).
...
Translate from `../noisemaker-hlsl/unity/com.noisemaker.hlsl/Shaders/Include/NMCore.hlsl` (HLSL→GLSL: `asuint`→`floatBitsToUint`, `(uint3)v`→`uvec3(v)`, `float3`→`vec3`). Contents: `pcg(uvec3)`, `prng(vec3)`, `random(vec2)`, `nm_mod(float,float)`, `nm_positiveModulo(int,int)`, `map(...)`, `periodicFunction(float)`. PCG divisor literal `4294967295.0`.
